...
### 2.11 Animation biến đổi biểu thức đại số (term morphing) — 2 chiều

> **Bối cảnh phát sinh:** Bài Nguyên hàm (đại số, không đồ thị/canvas), cần cho học sinh THẤY
> 1 biểu thức biến hình thành biểu thức khác theo 1 quy tắc (VD đạo hàm: 2x²-5x → 4x-5; nguyên
> hàm: 4x-5 → 2x²-5x + C), thay vì chỉ hiện kết quả tĩnh cạnh nhau. Chưa có pattern nào trong file
> này (PHẦN 2-3 gốc chỉ có đồ thị/miền/điểm — không có "biến đổi ký hiệu") xử lý được nhu cầu này,
> nên bổ sung tại đây làm chuẩn dùng chung cho mọi module đại số có tính chất "phép biến đổi 2
> chiều trên biểu thức" (đạo hàm ⇄ nguyên hàm, khai triển ⇄ rút gọn, quy đồng ⇄ tối giản...).

**Khi nào dùng pattern này:** biểu thức A biến thành biểu thức B qua 1 quy tắc rõ ràng, áp dụng
được TỪNG SỐ HẠNG độc lập (không phải phép biến đổi toàn cục kiểu giải phương trình nhiều bước).
Nếu phép biến đổi cần nhiều bước trung gian (không quy về "áp quy tắc 1 lần cho từng số hạng"),
dùng Step-by-step reveal thường (xem `01_scenario_builder_v4_1.md`) thay vì pattern này.

**1. Biểu diễn biểu thức dưới dạng mảng số hạng (term list) — KHÔNG dùng chuỗi text tĩnh:**

```javascript
// Mỗi số hạng: hệ số + biến + số mũ, đủ để tính lại và animate riêng từng phần
function term(coef, power) { return { coef, power }; }

// 2x² - 5x  →  [ {coef:2, power:2}, {coef:-5, power:1} ]
const F = [ term(2, 2), term(-5, 1) ];

// Quy tắc biến đổi THUẦN (không đụng DOM) — tách riêng để test Node theo đúng Mục 2.10
function differentiateTerm(t) {
  if (t.power === 0) return null; // hằng số biến mất
  return term(t.coef * t.power, t.power - 1);
}
function antiderivativeTerm(t) {
  return term(t.coef / (t.power + 1), t.power + 1);
}
```

> ⚠️ Bắt buộc viết `differentiateTerm`/`antiderivativeTerm` (hoặc quy tắc tương ứng của module
> khác) THUẦN JS, tách khỏi DOM, rồi verify bằng Node theo đúng quy trình Mục 2.10 — trước khi nối
> vào animation. Animate sai 1 hệ số vẫn "chạy mượt" về mặt hiệu ứng nhưng dạy sai kiến thức, và
> lỗi kiểu này KHÔNG lộ ra khi chỉ nhìn hiệu ứng chạy đẹp.

**2. Animate từng số hạng bằng cách đổi nội dung DOM có transition, không vẽ lại canvas:**

```javascript
// Mỗi số hạng là 1 <span> riêng, transition đổi trong CHÍNH span đó — không tạo/xoá node
// để tránh giật layout. Dùng transform+opacity (không animate width/left — layout thrashing).
function morphTerm(spanEl, fromTerm, toTerm, durationMs = 500) {
  spanEl.style.transition = `opacity ${durationMs/2}ms ease`;
  spanEl.style.opacity = 0;
  setTimeout(() => {
    spanEl.textContent = formatTerm(toTerm); // formatTerm(): "2x²" / "-5x" / "+C"...
    spanEl.style.opacity = 1;
  }, durationMs/2);
}
```

**3. Số hạng biến mất (hằng số bị đạo hàm) hoặc xuất hiện mới (+C khi lấy nguyên hàm):**
Không được để "biến mất đột ngột" hoặc "hiện đột ngột" — cả 2 trường hợp đều dùng fade
(opacity 1→0 rồi bỏ khỏi layout / opacity 0→1 sau khi đã có trong layout), KHÔNG dùng `display:none`
đổi tức thì.

**4. Điều khiển tốc độ — áp đúng "QUY TẮC ANIMATION/AUTO-PLAY BẮT BUỘC" của
`01_scenario_builder_v4_1.md`:** animation chỉ chạy khi học sinh chủ động bấm (nút chiều
đạo hàm / nút chiều nguyên hàm), KHÔNG tự động chạy liên tục; học sinh bấm lại được nhiều lần
(replay) — mỗi lần bấm phải reset đúng về trạng thái gốc trước khi chạy lại, không cộng dồn
state cũ (VD bấm 2 lần liên tiếp không được để "+C" xuất hiện 2 lần).

**5. Mobile:** nút bấm 2 chiều (VD "F'(x)" / "∫...dx") theo đúng chuẩn tap-to-toggle vùng chạm
≥44px ở Mục 3.8 — không dùng hover để trigger animation trên bất kỳ thiết bị nào.
...
### 3.6c Thanh tab ĐỘC LẬP (không canvas chung) — lướt ngang trên mobile

> **Thêm 08/2026** — phục vụ các module dạng "đọc thêm/khám phá" có nhiều
> nhánh nội dung KHÔNG tuần tự, KHÔNG khoá dần, và mỗi nhánh TỰ CHỨA nội
> dung riêng (minh hoạ + đọc + mô hình riêng) — KHÁC PHẦN 3.6b ở điểm cốt
> lõi: 3.6b có 1 canvas/đồ thị DUY NHẤT dùng chung cho mọi bước, phải giữ
> cố định trên cùng khi cuộn qua các thẻ bước; ở đây KHÔNG có canvas
> chung nào cần giữ cố định — mỗi tab là 1 khối nội dung độc lập hoàn
> toàn, chuyển tab là thay toàn bộ khối nội dung, không phải chỉ đổi 1
> phần bên cạnh canvas cố định.
>
> ⚠️ Đừng nhầm 2 pattern này khi build: nếu nội dung có 1 canvas cố định
> dùng lại qua nhiều bước → dùng 3.6b. Nếu mỗi tab là 1 "bài đọc" riêng,
> không chia sẻ canvas nào → dùng 3.6c (đơn giản hơn nhiều, không cần
> `order` hay giữ canvas cố định).

```html
<div class="tab-bar-standalone" id="tabBarStandalone">
  <button class="tab-pill active" data-tab="t1">Đo đạc</button>
  <button class="tab-pill" data-tab="t2">Thiên văn</button>
  <button class="tab-pill" data-tab="t3">Con quay</button>
  <!-- ... thêm tab tuỳ nội dung -->
</div>
<div class="tab-content-standalone">
  <section class="tab-panel active" data-panel="t1"><!-- minh hoạ + đọc + 3D riêng của tab 1 --></section>
  <section class="tab-panel" data-panel="t2"><!-- ... --></section>
  <!-- mỗi panel ẨN/HIỆN bằng display:none/block khi đổi tab, KHÔNG cần
       clearScene() dùng chung — nếu panel có canvas 3D riêng, canvas đó
       chỉ init khi panel active lần đầu (lazy init), dispose khi panel
       bị ẩn nếu cần giải phóng bộ nhớ (không bắt buộc như 3.4-BIS vì đây
       là canvas RIÊNG của từng tab, không phải canvas CHUNG bị vẽ chồng) -->
</div>
```
...
.tab-bar-standalone{
  ...
  scrollbar-width:none; padding:10px 12px; background:var(--cream);
  border-bottom:1px solid var(--paper-line); position:sticky; top:0; z-index:30;
}
.tab-bar-standalone::-webkit-scrollbar{ display:none; }
.tab-pill{
  flex:0 0 auto; padding:9px 16px; min-height:44px; border-radius:999px;
  border:1px solid var(--paper-line); background:var(--cream-2); color:var(--ink-2);
  font-size:12.5px; font-weight:600; white-space:nowrap; cursor:pointer;
}
.tab-pill.active{ background:var(--jade-pale); color:var(--jade-text); border-color:var(--jade-soft); }

.tab-panel{ display:none; padding:16px; }
.tab-panel.active{ display:block; }

/* Desktop (≥1024px) — tuỳ chọn: hiện dạng lưới thẻ để click mở, xem PHẦN
   mô tả module cụ thể. Mobile luôn dùng tab-bar-standalone như trên,
   không cần media query riêng vì bản chất đã là "lướt ngang" ở mọi kích
   thước màn hình (không bắt buộc chỉ mobile mới lướt ngang, khác 3.6b
   nơi desktop thường có đủ chỗ hiện tất cả thẻ bước không cần cuộn). */
```

**Checklist riêng cho 3.6c:**
- [ ] Thanh tab dùng `position:sticky` để luôn thấy được khi cuộn nội
      dung dài của tab đang mở (không bắt buộc `order` như 3.6b, vì
      không có canvas chung cần ưu tiên vị trí)
- [ ] Mỗi tab-pill ≥44px chiều cao (vùng chạm), dù chữ ngắn
- [ ] Không dùng modal/popup che toàn màn hình trên mobile cho pattern
      này — luôn hiện nội dung tab ngay dưới thanh tab, cuộn dọc bình
      thường
...
> **Phiên bản 2.5** — thêm Mục 3.6c: thanh tab ĐỘC LẬP (không canvas
> chung), lướt ngang trên mobile — phát sinh từ Module mở rộng Bài 22-24
> (đọc thêm/khám phá, 6 nhánh không tuần tự). Khác Mục 3.6b (dành cho
> sidebar bước khoá dần + 1 canvas chung phải giữ cố định) — 3.6c đơn
> giản hơn, mỗi tab tự chứa nội dung riêng, không cần `order` hay giữ
> canvas cố định khi chuyển tab. Dùng `position:sticky` cho thanh tab,
> tab-pill dạng pill với `.active` = jade-pale/jade-text.

> **Phiên bản 2.4** — thêm Mục 2.11: pattern animation biến đổi biểu thức đại số (term morphing)
> 2 chiều — phát sinh từ Module 1, Bài 11 Nguyên hàm (canvas biến đổi F(x) ⇄ f(x) qua đạo hàm/
> nguyên hàm). Biểu diễn biểu thức dưới dạng mảng số hạng (không chuỗi text tĩnh), quy tắc biến
> đổi tách riêng khỏi DOM để verify bằng Node theo Mục 2.10, animate qua opacity/transition trên
> từng `<span>` số hạng, không tự động chạy (chỉ chạy khi bấm, replay được, reset đúng state).
